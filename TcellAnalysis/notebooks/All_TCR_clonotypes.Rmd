---
title: "TCR clonotype definitions: All"
...
```{r, warning=FALSE, message=FALSE}
tcr.all <- read.table("~/Dropbox/COVID19/Data/TCR/TCR_merged.tsv",
                      sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
```{r, warning=FALSE, message=FALSE}
tcr.dist.df <- read.table("~/Dropbox/COVID19/Data/TCR/TCR_clonotypes.tsv",
                          sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
clon.freq.merge <- clon.freq.merge[!clon.freq.merge$D0_status_summary %in% c("Non_covid", "LPS"), ]
clon.freq.merge$D0_status_summary <- factor(clon.freq.merge$D0_status_summary,
...
clon.size.merge$Clone.Size <- factor(clon.size.merge$Clone.Size,
                                     levels=c("1", "2", "3", ">3"))
clon.size.merge <- clon.size.merge[!clon.size.merge$D0_status_summary %in% c("Non_covid"), ]

clon.size.merge$D0_status_summary <- factor(clon.size.merge$D0_status_summary,
...
ggplot(clon.size.merge[clon.size.merge$Clono.Counts.Freq > 1 &
                           !clon.size.merge$D0_status_summary %in% c("LPS") &
    ...
    facet_wrap(~D0_status_summary, scales="free_y", nrow=2) +
    #scale_y_continuous(limits=c(0, 100), oob=squish) +
    guides(fill=guide_legend(title="Cell type")) +
    labs(y="Clone size") +
    ggsave("~/Dropbox/COVID19/plot.dir/All_TCRclonotype_freq-boxplots.pdf",
...
ggplot(clon.size.merge[clon.size.merge$Collection_Day %in% c("D0") & 
       ...
       aes(x=D0_status_summary, fill=Clone.Size)) +
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/TCR_clonality_bars.pdf",
...
ggplot(clon.size.merge[clon.size.merge$Collection_Day %in% c("D0") & 
    ...
    theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1, size=14, colour="black"),
          ...
          strip.text=element_text(size=14, colour='black')) +
    guides(fill=guide_legend(title="Clone size")) +
    ggsave("~/Dropbox/COVID19/plot.dir/TCR_clonality_bars-gender.pdf",
...
ggplot(clon.size.merge[clon.size.merge$Collection_Day %in% c("D0") & 
    ...
    theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1, size=14, colour="black"),
          ...
          strip.text=element_text(size=14, colour='black')) +
    guides(fill=guide_legend(title="Clone size")) +
    ggsave("~/Dropbox/COVID19/plot.dir/TCR_clonality_bars_nosingleton.pdf",
           height=2.95,width=6.15, useDingbats=FALSE) +
    NULL
```
...
```{r}
expanded.naive <- tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid") &
...
write.table(unique(expanded.naive$CellID),
            file="~/Dropbox/COVID19/Data/Expanded_Naive.txt",
            sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE)

table(expanded.naive$D0_status_summary)
```
...
```{r}
sf.df <- read.table("~/Dropbox/COVID19/Data/Tcell_sizefactors.tsv",
                    sep="\t", header=TRUE, stringsAsFactors=FALSE)
sf.merge <- merge(sf.df, tcr.group.merge[tcr.group.merge$Sub.Annotation %in% c("CD4.Naive", "CD8.Naive"), ], by='CellID')
```

```{r}
ggplot(sf.merge, aes(x=Sub.Annotation, y=SFactor, fill=Expand.Clone)) +
    geom_boxplot() +
    theme_cowplot() +
    labs(x="Annotation", y="Size factor") +
    ggsave("~/Dropbox/COVID19/plot.dir/Expanded_Naive_sizeFactor-boxplot.pdf",
           height=1.95, width=4.15, useDingbats=FALSE)
```
...
tcr.group.merge$OrderedSeverity <- tcr.group.merge$D0_status_summary
...
ggplot(tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS", "Healthy") &
                           tcr.group.merge$Collection_Day %in% c("D0") &
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/Tcells_CloneSizeDistribution-collapse-bars.pdf",
...
expand.tab <- as.data.frame(xtabs( ~ Expand.Clone + sample_id + Sub.Annotation, 
                                   data=tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS") &
                                                            tcr.group.merge$Collection_Day %in% c("D0") &
                                                            tcr.group.merge$Expand.Clone %in% c("Expanded") &
                                                            !tcr.group.merge$Sub.Annotation %in% c("gdT", "MAIT", "NKT"), ]))
...
expand.freq.merge$OrderedSeverity <- expand.freq.merge$D0_status_summary
...
ggplot(expand.freq.merge[!expand.freq.merge$Sub.Annotation %in% c("NKT", "gdT", "MAIT", "CD4.Naive", "CD8.Naive"), ], 
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/Expanded_CloneSize_distribution-boxplot.pdf",
...
ggplot(expand.freq.merge[!expand.freq.merge$Sub.Annotation %in% c("NKT", "gdT", "MAIT", "CD4.Naive", "CD8.Naive"), ], 
    ...
    theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1),
          strip.background=element_rect(fill='white', colour='white')) +
    guides(fill=guide_legend(title="Severity")) +
    ggsave("~/Dropbox/COVID19/plot.dir/Expanded_CloneSize_distribution-boxplot-high_level.pdf",
...
expand.tab.merge$OrderedSeverity <- expand.tab.merge$D0_status_summary
...
ggplot(tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS") &
                           tcr.group.merge$Collection_Day %in% c("D0") &
    ...
    facet_wrap(~OrderedSeverity, nrow=1) +
    labs(x="Clone Size", y="Proportion") +
    ggsave("~/Dropbox/COVID19/plot.dir/TCR_clonal_expansion-phenotype.pdf",
    ...
    theme(aspect=5/3,
          ...
          strip.text=element_text(size=10, face='bold')) +
    guides(fill=guide_legend(title="Cell type", ncol=1)) +
    ggsave("~/Dropbox/COVID19/plot.dir/Tcells_CloneSizeDistribution-bars.pdf",
           height=2.95, width=9.95, useDingbats=FALSE) +
    NULL
```
...
expanded.tabs <- table(tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS", "Healthy") &
                    tcr.group.merge$Collection_Day %in% c("D0") &
                    !tcr.group.merge$Sub.Annotation %in% c("gdT", "NKT", "MAIT"), ]$Sub.Annotation, 
                    tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS", "Healthy") &
                                        tcr.group.merge$Collection_Day %in% c("D0") &
...
for(x in seq_along(ctypes)){
    x.c <- ctypes[x]
    x.dat <- tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid", "LPS", "Healthy") &
                           tcr.group.merge$Collection_Day %in% c("D0") &
                           tcr.group.merge$Sub.Annotation %in% x.c, ]
    x.dat$OrderedSeverity <- ordered(x.dat$D0_status_summary,
...
ggplot(clone.lm.df, 
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/TCR-ExpandClones-logLM_ordered.pdf",
...
clon.lm.pvalues <- data.frame("CellType"=loglm.table$Celltype, "OR"=loglm.table$OR,
                              "LCI"=loglm.table$LCI, "UCI"=loglm.table$LCI, "Pvalue"=loglm.table$Pvalue,
                              "FDR"=loglm.table$Padjust)

kbl(clon.lm.pvalues) %>% kable_paper(full_width=FALSE) %>% 
    save_kable("~/Dropbox/COVID19/plot.dir/Clonality_logLM_trend_pvals.html",
               self_contained=TRUE)
```
...
```{r, message=FALSE, warning=FALSE}
ctypes <- c("CD4.CM", "CD4.EM", "CD4.IL22", "CD4.Prolif", "CD4.Th1", "CD4.Th2", "CD4.Tfh",
...
ggplot(clone.lm.df.nosite[!clone.lm.df.nosite$Predictor %in% c("OrderedSeverity^5", "OrderedSeverity^4", "OrderedSeverity.C"), ], 
    ...
    scale_fill_manual(values=c("blue", "orange")) +
    facet_wrap(~Celltype, scales="free_x")  + 
    ggsave("~/Dropbox/COVID19/plot.dir/TCR-ExpandClones-logLM_ordered-noSite.pdf",
           height=5.95, width=9.95, useDingbats=FALSE)
```
...
clone.size.cd8 <- as.data.frame(xtabs(~sample_id + Sub.Annotation,
                                      data=tcr.group.merge[!tcr.group.merge$D0_status_summary %in% c("Non_covid") &
                                                               tcr.group.merge$Expand.Clone %in% c("Expanded"), ]))
...
clone.size.cd8.melt$OrderedSeverity <- clone.size.cd8.melt$D0_status_summary
...
write.table(clone.size.cd8.melt,
            file="~/Dropbox/COVID19/Data/ExpandedClone_counts.tsv",
...
ggplot(clone.size.cd8.melt[clone.size.cd8.melt$variable %in% c("CD8.EM", "CD8.TE") &
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/Tcell_CD8_ExpandedClones.pdf",
...
ggplot(clone.size.cd8.melt[clone.size.cd8.melt$Collection_Day %in% c("D0") &
    ...
    guides(fill=guide_legend(title="Severity")) +
    scale_y_continuous(limits=c(0, q.95[2]), oob=squish) +
    ggsave("~/Dropbox/COVID19/plot.dir/Tcell_All_ExpandedClones.pdf",
...
write.table(te.em.df,
            file="~/Dropbox/COVID19/Data/TE-EM_ratio.tsv",
            quote=FALSE, sep="\t", row.names=FALSE)
te.em.df <- merge(te.em.df, covid.meta, by='sample_id')

te.em.df$OrderedSeverity <- te.em.df$D0_status_summary
te.em.df <- te.em.df[!te.em.df$D0_status_summary %in% c("LPS", "Healthy"), ]
...
ggplot(te.em.df[te.em.df$Collection_Day %in% c("D0"), ],
    ...
    scale_y_continuous(limits=c(0, q.95[2]), oob=squish) +
    guides(fill=guide_legend(title="Severity")) +
    ggsave("~/Dropbox/COVID19/plot.dir/Tcell_CD8_TE-EMRatio-counts.pdf",
...
clon.size.dist$D0_status_summary <- factor(clon.size.dist$D0_status_summary,
                                           levels=c("Healthy", "Asymptomatic", "Mild", "Moderate", "Severe", "Critical", "LPS"))
...
ggplot(clon.freq.merge[clon.freq.merge$Sub.Annotation %in% select.cells, ],
       aes(x=TCR.Gini, y=1-Prop.Clonotypes, colour=D0_status_summary)) +
    geom_point() +
    theme_cowplot() +
    scale_colour_manual(values=group.cols) +
    facet_wrap(~Sub.Annotation) +
    labs(x="TCR Chain", y="Clonalality") +
    ggsave("~/Dropbox/COVID19/plot.dir/TCR_clonotype_ClonalityVsGin.png",
...
```{r, fig.height=5.95, fig.width=9.95}
ggplot(clon.freq.merge[clon.freq.merge$Agg.Cell %in% c("CD4.Mem", "CD4.Naive", "CD4.Tfh", "CD4.Th1", "CD4.IL22",
                                                       "CD4.Prolif", "CD8.Prolif",
                                                       "CD8.Mem", "CD8.Naive", "CD8.TE", "Treg"), ],
       aes(x=D0_status_summary, y=TCR.Gini, fill=D0_status_summary)) +
    geom_boxplot() +
    theme_cowplot() +
    scale_fill_manual(values=group.cols) +
    ...
    guides(fill=guide_legend(title="Severity")) +
    NULL + 
    ggsave("~/Dropbox/COVID19/plot.dir/All_TCRclonality-boxplot_severity.pdf",
           height=5.95, width=9.95, useDingbats=FALSE)
```
...
a higher clonal diversity. How do other factors that contribute to clonality affect this, for instance age?
...
ggplot(clon.freq.merge[clon.freq.merge$Agg.Cell %in% c("CD4.Mem", "CD4.Naive", "CD4.Tfh", "CD4.Th1", "CD4.Th2", "CD4.Th17", "CD4.IL22",
                                                       "CD4.Prolif", "CD8.Prolif",
                                                       "CD8.Mem", "CD8.Naive", "CD8.TE", "Treg"), ],
       aes(x=Age, y=TCR.Gini, colour=D0_status_summary)) +
    geom_point() +
    ...
    NULL +
    ggsave("~/Dropbox/COVID19/plot.dir/All_TCRclonality-age.pdf",
           height=5.95, width=9.95, useDingbats=FALSE)
```
...
ggplot(clon.freq.merge[clon.freq.merge$Agg.Cell %in% c("CD8.Mem", "CD8.TE"), ],
       aes(x=D0_status_summary, y=TCR.Gini, fill=Sex)) +
    geom_boxplot(outlier.size=0.5) +
    theme_cowplot() +
    ...
    NULL + 
    ggsave("~/Dropbox/COVID19/plot.dir/All_TCRclonality-boxplot_severity_gender.pdf",
           height=2.95, width=7.95, useDingbats=FALSE)
```
...
covid.freq.merge$OrderedStatus <- ordered(covid.freq.merge$D0_status_summary,
                                          levels=c("Healthy", "Asymptomatic", "Mild", "Moderate", "Severe", "Critical"))

c.types <- setdiff(unique(covid.freq.merge$Agg.Cell), c("CD4.Th17", "NKT", "gdT", "MAIT"))
for(i in seq_along(c.types)){
    i.c <- c.types[i]
    i.clono.tcr.lm <- lm(1 - Prop.Clonotypes ~ Age + Site + Sex + OrderedStatus, 
                             data=covid.freq.merge[covid.freq.merge$Agg.Cell %in% c(i.c), ])
...
clono.lm.df$Predictor <- gsub(clono.lm.df$Predictor, pattern="D0_status_summary", replacement="")
```
...
kbl(clon.lm.pvalues) %>% kable_paper(full_width=FALSE) %>% 
    save_kable("~/Dropbox/COVID19/plot.dir/TCR_LM_trend_pvals.html",
               self_contained=TRUE)
```
...
ggplot(clono.lm.df[!clono.lm.df$Predictor %in% c("OrderedStatus^5", "OrderedStatus^4", "OrderedStatus.C"), ], 
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/All_TCRclonality-LM_ordered.pdf",
...
exp.clon.freq.merge <- exp.clon.freq.merge[!exp.clon.freq.merge$D0_status_summary %in% c("Non_covid", "LPS"), ]
exp.clon.freq.merge$D0_status_summary <- factor(exp.clon.freq.merge$D0_status_summary,
                                            levels=c("Healthy", "Asymptomatic", "Mild", "Moderate", "Severe", "Critical"))
...
```{r, fig.height=5.95, fig.width=9.95}
ggplot(exp.clon.freq.merge[exp.clon.freq.merge$Sub.Annotation %in% c("CD4.Mem", "CD4.Naive", "CD4.Tfh", "CD4.Th1", "CD4.IL22",
                                                                     "CD4.Prolif", "CD8.Prolif",
                                                                     "CD8.Mem", "CD8.Naive", "CD8.TE", "Treg"), ],
       aes(x=D0_status_summary, y=TCR.Gini, fill=D0_status_summary)) +
    geom_boxplot() +
    ...
    guides(fill=guide_legend(title="Severity")) +
    NULL + 
    ggsave("~/Dropbox/COVID19/plot.dir/ExpandedClones_TCRclonality-boxplot_severity.pdf",
           height=5.95, width=9.95, useDingbats=FALSE)
```
...
exp.covid.freq.merge$OrderedStatus <- ordered(exp.covid.freq.merge$D0_status_summary,
                                          levels=c("Asymptomatic", "Mild", "Moderate", "Severe", "Critical"))
...
ggplot(clono.lm.df[!clono.lm.df$Predictor %in% c("OrderedStatus^5", "OrderedStatus^4", "OrderedStatus.C"), ], 
    ...
    scale_fill_manual(values=c("blue", "orange")) +
    facet_wrap(~Agg.Cell, scales="free_x")  + 
    ggsave("~/Dropbox/COVID19/plot.dir/Expanded_TCRclonality-LM_ordered.pdf",
           height=8.95, width=9.95, useDingbats=FALSE)
```
...
ggplot(clono.lm.df[!clono.lm.df$Predictor %in% c("OrderedStatus^5", "OrderedStatus^4", "OrderedStatus.C"), ], 
    ...
    ggsave("~/Dropbox/COVID19/plot.dir/Expanded-All_TCRclonality-LM_ordered.pdf",
